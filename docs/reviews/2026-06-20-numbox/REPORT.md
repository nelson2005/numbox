...
`numbox/utils/lowlevel.py:249-260`
...
`numbox/core/bindings/abi.py:95-96, 120-126`
...
- **Evidence:** `abi.py:96` bit-packs; the Record branch by contrast uses real `ty.size`/`fld.offset`. `call.py:131/145` use `context.get_value_type(...)` (the padded `LiteralStructType`) for the alloca/store/coercion. `test/core/test_abi.py:226-227` even pins `_struct_bytes(NamedTuple([int32, int64])) == 12`, whereas `{i32, i64}` is 16 bytes under natural alignment.
...
`numbox/core/bindings/_sqlite_tvf.py:456-463`
...
`numbox/utils/pysqlite_bridge.py:110-164`
...
`numbox/core/bindings/_sqlite_tvf.py:192-202`
...
`numbox/core/bindings/abi.py:96, 122-124`
...
`numbox/core/bindings/_fmtio.py:210-227, 271-301` (and the SEC face at `316-317, 547-548`)
...
`numbox/core/variable/variable.py:386-402`
...
`numbox/core/variable/variable.py:94-105, 494, 579`
...
`numbox/utils/highlevel.py:123-126`
...
1. **repr-vs-source mismatch** — `params_str` is built via `repr()` of each default (`f'{k}={v.default!r}'`,
   `standard.py:46`) while `params_str_` is the raw source captured by the header regex. `assert params_str
...
`numbox/core/proxy/proxy.py:87-93`
...
`numbox/core/work/builder.py:73-77, 106-116, 170-184`
...
`numbox/core/work/explain.py:8-10`
...
`numbox/core/work/node.py:81-88` and `numbox/core/work/work.py:356-364`
...
`numbox/core/bindings/_fmtio.py:154-161`
...
`numbox/core/bindings/signatures.py:173`
...
`numbox/core/configurations.py:5-19`
...
`numbox/core/proxy/proxy.py:80-81`
...
`numbox/core/variable/node.py:22-31`
...
`numbox/core/vector/vector.py:104-117`
...
`numbox/core/work/builder.py:108`
...
`numbox/core/work/node.py:101-148`
...
`numbox/utils/fingerprint.py:55-61`
...
`numbox/utils/pysqlite_bridge.py:93-107`
...
`numbox/utils/standard.py:45-47`
...
`numbox/core/bindings/_fmtio.py:210-227, 316-317, 547-548`
...
`numbox/core/bindings/_fmtio.py:387-415, 667-686`
...
`numbox/core/bindings/utils.py:109-163`
...
**DES-m1 — UTF-8 decode state machine duplicated** (`_sqlite_query.py:32-71`). `_put_unicode` reimplements
...
**DES-m2 — 14-branch tag→sqlite3_result ladder duplicated across three cfunc bodies** (`_sqlite_tvf.py:308-343`).
...
**DES-m3 — Case-B eager segment loop duplicates `_discover_and_run`'s orchestration** (`compile_kernel.py:906-934`).
...
**DES-m4 — Four-way duplication of the registry-cached codegen skeleton** (`work.py:219-239,287-304,336-353,402-421`).
...
**DES-m5 — Asymmetric, partially-undocumented `removerefctpass` defense in `meminfo.py`** (`meminfo.py:77-111`).
...
**DES-m6 — `timer.py` mutates the root logger at import and logs routine timing at WARNING** (`timer.py:5-21`).
...
- **Dead state / dead params:** `work.py:169,211-216` write-only `_source_getter_registry` (DES-work-1);
  `builder.py:124-132` dead `registry` param on `_infer_end_and_derived_nodes` (DES-work_builder-1);
  `builder.py:25-26,50-53` `registry` field stored-never-read on End/Derived (DES-work_builder-2);
  `_kernel_partition.py:168-197` dead `external` param on `segment_liveness` (DES-kernel_partition-1);
  `combine_utils.py:42-51` unused `field_ind` enumerate index (DES-work_combine_loader_lowlevel-2);
  `compile_kernel.py:195-196` `in_names` computed only for a source comment (DES-compile_kernel-3).
- **Duplication to factor:** `lowlevel.py:58-61,…` verbatim pointer-type TypingError guard ×4 →
  `_require_intp_pointer` helper (DES-lowlevel-1); `lowlevel.py:56-152` aligned/unaligned load+store codegen
  differs only by `align=1` — keep the public 4-way split, factor the inner codegen (DES-lowlevel-2);
  `_kernel_partition.py:168-228` `segment_liveness`/`cone_liveness` share live-in logic (DES-kernel_partition-2);
  `_sqlite_query.py:113-126` `_TAG_S`/`_TAG_BLOB` differ only by accessor (DES-sqlite_query-2);
  `_fmtio.py:237-259,504-518` `%n`-rejection ×2 (DES-libc_fmtio-1); `_fmtio.py:401-405,677-681` sscanf
  ...
  identical init+finalize branches (DES-sqlite_udf-1); `_sqlite_tvf.py:146-289` xConnect/xDisconnect/xRowid/
  xEof boilerplate re-implemented vs the vtable module (DES-sqlite_tvf-3); `call.py:176-177,…` `{i64,i64}`
  literal struct reconstructed ×3 (DES-call-1); `bindings/utils.py:159-164` per-platform CDLL block ×2
  (DES-bindings_utils-1); `any_type.py:63-66,77-82` payload-wrap idiom ×2 (DES-any-1); `vector.py:120-141`
  grow-and-copy block ×2 (DES-vector-1); `work/node.py:101-148` traversal body duplicated module-helper vs
  overload (DES-work_node-1); `work.py:367-374,454-465` `get_inputs_names`/`depends_on` copy-pasted Work↔Node
  (DES-work-4); `compile_kernel.py:454-462,917-919` live-in computed two ways (DES-compile_kernel-2),
  `:464` duplicated sort-key lambda (DES-compile_kernel-4); `pysqlite_bridge.py:106,148` version-string
  computed twice (DES-pysqlite_bridge-1); `_errno.py:35` inconsistent size_t/intp construction across
  siblings (DES-libc_stdio_strerror_errno-1).
- **Single-use wrapper / API hygiene:** `_sqlite_vtable.py:730-739` `_VTableHandle` is a redundant single-use
  wrapper around the already-keep-alive `_BuiltDescriptor` (DES-sqlite_vtable-1), and its `__init__` takes
  variadic `*objs` but is always called with one (DES-sqlite_vtable-2); `print_tree.py:15-55` internal helpers
  lack a leading underscore and `make_graph` shadows the public `builder.make_graph` (DES-work_print_explain-1);
  `explain.py:24-25` bare magic `[1]` index into `work.derive` (DES-work_print_explain-2);
  `combine_utils.py:57-66` overload name contradicts behavior (copy-paste from loader_utils)
  (DES-work_combine_loader_lowlevel-1); `proxy.py:48-49` self-referential `dict.update` obscures
  `inline='always'` (DES-proxy-1); `_math.py` wrappers `round`/`pow` shadow builtins in the star-imported
  public namespace (DES-libc_math-1); `_fmtio.py:682-685,461-467` sscanf threads an unused
  `get_unicode_data_p` arg (DES-libc_fmtio-3); `_strerror.py:95-114` test-only IR-probe helper lives in the
  production module (DES-libc_stdio_strerror_errno-2); `_sqlite_tvf.py:85,403` `_TVF_DESC_DTYPE.itemsize`
  written never read (DES-sqlite_tvf-2); `_sqlite_typemap.py:66-105` public `utf8_to_utf32` has no production
  caller (only its `_sqlite_query` twin is live) (DES-sqlite_typemap_constants-1); `abi.py:155-160` docstring
  attributes orchestration to the wrong caller, `:165-184` loop var `size` shadows the outer `size`
  (DES-abi-1, DES-abi-2); `highlevel.py:68-70,242-247` `hash_type`/`prune_type` are unused public surface,
  `:84` identity list-comprehension, `:219-234` six redundant exec-ns entries (DES-highlevel-1/3/2);
  `work.py:346,414` redundant ns overrides re-inject module-level names (DES-work-2), `:460-465`
  assert-after-def placement inconsistent with the Node sibling (DES-work-5); `variable/utils.py:1-7`
  module docstring omits one helper concern (DES-variable_node_utils-1); `variable.py:391-396` O(N) linear
  scan over `ordered_nodes` (DES-variable-2), `:478-479` dead defensive string-normalization branch
  (DES-variable-1).
- **Config object-ness:** `configurations.py:5-19` validates JSON syntax but not dict-ness (DES-configurations-1)
  — folded into COR-11.
...
**TST-m1 — register_tvf string/unicode/blob output columns are never tested** (`_sqlite_tvf.py:330-343` plus
...
**TST-m3 — Windows DLL search-order resolution has no test** (`bindings/utils.py:49-112`). `_resolve_lib_path`
...
**TST-m4 — No musl runner; strerror_safe musl path covered only by a glibc IR probe** (`numbox_ci.yml:25,33-86`).
...
**TST-m5 — `_strerror` docstring asserts a CI canary that does not exist** (`_strerror.py:24-26`). The module
...
**TST-m6 — Sphinx build is not gated on PRs and tolerates all warnings** (`docs.yml:3-5,28-29`). `docs.yml`
...
**TST-m7 — pysqlite_bridge docs claim a non-existent RTLD_GLOBAL import-preload** (`docs/numbox.utils.rst:210-217`).
...
**TST-m8 — Doc says Py_DEBUG builds are unsupported, but the code supports them** (`docs/numbox.utils.rst:219-221`).
...
**TST-m9 — `_sqlite_typemap` module entirely missing from sphinx docs** (`docs/numbox.core.bindings.rst:19-31,448-651`).
...
**TST-m10 — work.rst broken xref and dead graph-manager example** (`docs/numbox.core.work.rst:341,524-526,542`).
...
- **Doc drift:** `docs/numbox.core.variable.rst:129-133` references a nonexistent `.variables` attribute on
  the Variables namespace (TST-tst_variable_work-1); `docs/numbox.core.work.rst:473,480,199,408-412` public
...
- **Untested public surface:** `_c.py:50-144` `puts`/`fputc`/`putchar`/`feof`/`ferror`/`clearerr` have no test
  (TST-tst_bindings_abi_libc-3); `bindings/utils.py:13-46` `intp_ll_type`/`extract_literal_str` edge cases
  untested (TST-tst_bindings_abi_libc-6); `_fmtio.py:81-85` documented `%ld` Win64 truncation footgun has no
...
- **Untested edge cases:** `_sqlite_query.py:111-126` oversized-cell truncation (`nbytes >= width`) for
  TEXT/BLOB/U is untested (TST-tst_sqlite-3, paired with the `_sqlite_query` cluster);
  `_sqlite_typemap.py:78-105` truncated multi-byte sequence at end-of-buffer untested (TST-tst_sqlite-4);
  `test_sqlite_constants.py` asserts only the 8 index-constraint ops, leaving result/type/flag/destructor
  values unguarded (TST-tst_sqlite-5); `test_lowlevel.py:165-171` `get_str_from_p_as_int` tested on one
...
`numbox/core/bindings/_sqlite_query.py:25-29`
