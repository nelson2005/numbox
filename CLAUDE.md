...
- **SQLite `query_to_array` + vtable pushdown + TVFs (phase 5)** — merged 2026-06-11 via upstream [Goykhman/numbox#22](https://github.com/Goykhman/numbox/pull/22) at [`50df0f4`](https://github.com/Goykhman/numbox/commit/50df0f4) (fork review [PR #46](https://github.com/nelson2005/numbox/pull/46) — review vehicle only, never merged to fork main; full 25-comment maintainer-review disposition recorded in fork issue [#51](https://github.com/nelson2005/numbox/issues/51)). Three new modules — [`_sqlite_query.py`](numbox/core/bindings/_sqlite_query.py) (`query_to_array`: run SQL from `@njit` code and materialize the result as a 2-D float64 numpy array; raises on mid-step errors), [`_sqlite_tvf.py`](numbox/core/bindings/_sqlite_tvf.py) (`register_tvf`: table-valued functions backed by `@cfunc` kernels, with numeric-arg validation and a per-arg `argvIndex` bitmask in `xBestIndex`), [`_sqlite_typemap.py`](numbox/core/bindings/_sqlite_typemap.py) (packed-cell tag/spec map + unaligned UTF-32 stores with UTF-8 validation) — plus a major [`_sqlite_vtable.py`](numbox/core/bindings/_sqlite_vtable.py) expansion (read-only vtables over structured numpy arrays with eq/range constraint pushdown including bool columns via `xBestIndex`, `create_module_v2` with deferred `xDestroy`, `SQLITE_STATIC` for string/BLOB results), and align-1 [`load_unaligned`/`store_unaligned`](numbox/utils/lowlevel.py) helpers with IR-level `align 1` regression tests (`load_at`/`store_at` are UB on misaligned addresses).
  ...
  **Platform gotcha worth memorizing:** on macOS-arm64 (py3.14 / numba 0.65.1) raw-pointer result stores (`array_data_p` + `store_unaligned`) were dead-code-eliminated — `query_to_array` returned correct-shape all-zeros on that toolchain only. Fix: write results through `out.view(np.uint8)`, a numba-tracked view the optimizer can't drop, with native byte-order serialization via a typed view of an 8-byte scratch buffer.
  ...
  Follow-up: the `_query_core` trim comment was reworded to absolute slack bounds via upstream [PR #25](https://github.com/Goykhman/numbox/pull/25), merged 2026-06-12 at [`189c0f3`](https://github.com/Goykhman/numbox/commit/189c0f3) and synced into fork `main` via the 2026-06-13 sync PR.
- **compile_kernel (`Variable` graph → fused `@njit` kernel)** — merged 2026-06-15 via upstream [Goykhman/numbox#24](https://github.com/Goykhman/numbox/pull/24) at [`a7f18a7`](https://github.com/Goykhman/numbox/commit/a7f18a7) (feature commit [`d479cca`](https://github.com/Goykhman/numbox/commit/d479cca); supersedes closed [#23](https://github.com/Goykhman/numbox/pull/23); fork review [nelson2005/numbox#52](https://github.com/nelson2005/numbox/pull/52) closed unmerged — review vehicle only; earlier vehicle [#49](https://github.com/nelson2005/numbox/pull/49) closed unmerged). New `numbox/core/variable/compile_kernel.py`: `compile_kernel(graph, required)` compiles a `Variable` graph into one fused `@njit` kernel for the requested variables, alongside `core.work` — no per-node types needed (numba infers every interior type from runtime args; plain-Python formulas auto-wrapped with `njit()`). Reuses `Graph.compile` (topological order + external-variable discovery) and `utils/preprocessing`'s content-addressed anchor (structured fingerprint covering each formula's code, consts, defaults, closure cells, referenced globals/helpers, module, and jit flags; cached kernels reload cross-process). `CompiledKernel` exposes a bare `.kernel` hot path plus a dict-in/out `.execute` mirroring `CompiledGraph.execute`; generated identifiers are keyword- and newline-safe with minimal deterministic suffixes. v1 omits per-node `cacheable` memoization, incremental `recompute`, `None`-as-value formulas, and node-identity load/combine (use `CompiledGraph`/`Work`). Design + plan under `docs/superpowers/{specs,plans}/2026-06-07-compile-kernel*` (fork-only, not on `main`); tests in `test/core/test_compile_kernel.py`. **v2 segmented orchestration** (shipped to #24, 2026-06-13): graphs with non-jittable formulas now work — the first call probes each node against real values (numba compile errors demote a node to plain Python, runtime errors propagate) and a Python master orchestrates fused `@njit` segments around the demoted nodes; segment count is provably minimal (1 + max jit/Python alternations along any dependency path); `CompiledKernel.partition` reports the partition with per-node demotion reasons; per-segment content-addressed caching. v2 docs under `docs/superpowers/{specs,plans}/2026-06-12-compile-kernel-segments*` (fork-only). **Incremental recompute** (shipped to #24, 2026-06-15): `CompiledKernel.recompute(changed)` does a value-only refresh of just the cone affected by changed inputs — cone-based lazy re-fusion over a Python-side value store with dominator-based boundary persistence, a bounded LRU cone-plan cache, and type-change flush+reseed; benchmarks ~2.2x (chain suffix-cone) / ~4.1x (fan-out), with an honest diamond loss. Fingerprinting was reconciled into a shared `numbox/utils/fingerprint.py` (also backing `numbox/utils/digest.py`); helpers moved to `numbox/core/variable/utils.py`. Recompute docs under `docs/superpowers/{specs,plans}/2026-06-15-compile-kernel-recompute*` (fork-only).
- **Static `params`-driven jitability for `compile_kernel`** — merged 2026-06-18 via upstream [Goykhman/numbox#27](https://github.com/Goykhman/numbox/pull/27) at [`af6bed2`](https://github.com/Goykhman/numbox/commit/af6bed2) (fork review [nelson2005/numbox#58](https://github.com/nelson2005/numbox/pull/58), review vehicle only). Optional [`Variable.params`](numbox/core/variable/variable.py) (frozen `Params(jitable, type)`) makes jitability **declared** at build — a three-way [`_classify`](numbox/core/variable/compile_kernel.py) picks eager-fused / eager-segmented / lazy-#24-fallback — rather than **discovered** at first call (as #24 shipped); includes build-time declared-return validation and `recompute` for declared kernels. Design + plan under `docs/superpowers/{specs,plans}/2026-06-16-params-jitability*` (fork-only, not on `main`). The sync also picks up an unrelated upstream [`make_structref`](https://github.com/Goykhman/numbox/commit/ee8bb15) custom-globals improvement.
