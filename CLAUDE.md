...
- **Windows coverage of the variadic stdio path.** 20 tests in [`test_fmtio.py`](test/core/test_fmtio.py) (18) and [`test_stdio_handles.py`](test/core/test_stdio_handles.py) (2) carry `@pytest.mark.skipif(platform_ == "Windows", reason="capfd does not reliably capture C-level stdio writes on Windows")`. Treat that reason as a hypothesis, not an observation. It was added in the same commits that wrote the tests and never revisited; `capfd` is fd-level `dup2` capture (`_pytest/capture.py`, `FDCaptureBase`), which is the mechanism meant to catch C-level writes; and all 20 already call `fflush` before asserting, so plain buffering is ruled out. Coverage there is broad rather than absent: 47 of the 65 `test_fmtio.py` tests run on Windows, and `test_all_binding_families_survive_subprocess_round_trip` byte-verifies jitted `printf` against a real child process's stdout. Two gaps survive that. **No float ever traverses the variadic path on Windows CI**: every Windows-executed variadic call is `%d` or `%s`, while `%f`, the float32-to-double and int8/bool-to-int32 promotions and `%lld` appear only inside the skipped tests. That is the ABI class where Windows x64 requires a float duplicated into the shadow GP register, and where a regression is silently wrong output. **Nothing on Windows separates `stderr()` from `stdout()`**: the unskipped distinctness test proves the three handles differ but cannot catch a transposition, and the subprocess probe asserts on stdout only. Likely root cause, worth settling before any fix: `_resolve_lib_path("c")` returns `find_msvcrt()`, which is `None` on every VS2015+ CPython, so [`_load_lib_with_handle`](numbox/core/bindings/utils.py) falls back to `ctypes.cdll.msvcrt` (the legacy CRT) while `stdout()` emits `__acrt_iob_func` (a UCRT export). Two CRTs mean two FILE tables and two fd tables, so a legacy write handed a UCRT `FILE*` is undefined behaviour rather than merely uncaptured, and a second CRT that cached its fd-1 handle at DLL init would bypass a later `dup2` entirely. The `snprintf` access violation already recorded in [`fmtio.py`](numbox/core/bindings/fmtio.py) is the same shape. Settle it with a Windows-only informational test printing `ll.address_of_symbol` for `printf`/`fprintf`/`fputs`/`fflush`/`__acrt_iob_func` and which loaded module each matches; it returns `None` until numba has built an ExecutionEngine, so import the bindings first. Then close both gaps with no capture fixture at all: `snprintf` into a numpy buffer read back at the NUL for the formatting assertions, and a child process with separate stdout and stderr for the routing ones.
...
- **Ref-scoped concurrency on the `build-docs` job.** [`docs.yml`](.github/workflows/docs.yml) serialises only the publish: `deploy-docs` sits in a single `docs-deploy` group that is deliberately ref-independent, so deploys from any ref queue against one another and an older commit cannot overwrite a newer one on `gh-pages`. Nothing supersedes a `build-docs` run, so a second push to a pull request leaves the first Sphinx build running to completion for an artifact nothing will ever download, since `deploy-docs` declines on a pull request and the upload carries `retention-days: 1`. A job-level `concurrency` on `build-docs`, keyed the way [`numbox_ci.yml`](.github/workflows/numbox_ci.yml) already keys its own (`${{ github.workflow }}-${{ github.ref }}` with `cancel-in-progress: true`), would drop the superseded run. Keep it job-level: a workflow-level block would pull `deploy-docs` into the same ref-keyed group and break exactly the cross-ref serialisation described above. Efficiency only, no correctness consequence.
