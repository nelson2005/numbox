...
- **Extend `_call_lib_func` to support >16-byte struct returns.** Current `_CLASS_STRUCT_LARGE` returns raise `TypingError` (per [`call.py`](numbox/core/bindings/call.py) docstring: *"No consumer currently needs this; add it when one does."*). Consumer arrived: numbduck's `_duckdb_get_decimal` and `_duckdb_get_varint` hand-roll the same alloca-pointer-pass pattern `_call_lib_func` already does for `<=16B` on Windows. Generalizing the LARGE return path (caller-allocated hidden first arg, void return; mirrors existing Windows `<=16B` non-register-passable case) would let numbduck drop those locals. ABI helpers in [`abi.py`](numbox/core/bindings/abi.py) (`_classify`, `_struct_bytes`) already classify these correctly. Test: round-trip cases for a >16B return on SysV x86-64, AAPCS64, and Win64.
- **Add SysV x86-64 16B small-struct INT/INT eightbyte repack to `_call_lib_func`.** When a 16B struct passed by-value has both eightbytes classified as INTEGER but the LLVM type isn't already `{i64, i64}` (e.g. `{i32, i32, i64}`), llvmlite's lowering can drop fields because it doesn't model the eightbyte-packing rule. Fix: detect the pattern via per-eightbyte classification of the LLVM struct type — walk fields, classify each as INTEGER (integers, pointers) or SSE (float, double), group by eightbyte; if both eightbytes are pure INTEGER and the LLVM type isn't canonical, repack via memory bitcast (alloca + store original + load as `{i64, i64}`) before the call. Skip if already `{i64, i64}` or if either eightbyte contains SSE fields (those need a different ABI path). Mirrors the eightbyte rules underlying [`abi.py`](numbox/core/bindings/abi.py)'s existing `_classify`. Unblocks numbduck's interval (`{i32, i32, i64}`) migration to `_call_lib_func` — currently handled by a numbduck-local `_build_packed_interval` workaround that should live in numbox where the rest of the SysV ABI handling does.
