...
- **compile_kernel (`Variable` graph → fused `@njit` kernel)** — upstream [Goykhman/numbox#23](https://github.com/Goykhman/numbox/pull/23) OPEN (fork review [nelson2005/numbox#49](https://github.com/nelson2005/numbox/pull/49) — review vehicle only, closed unmerged). New `numbox/core/variable/compile_kernel.py`: `compile_kernel(graph, required)` compiles a `Variable` graph into one fused `@njit` kernel for the requested variables, alongside `core.work` — no per-node types needed (numba infers every interior type from runtime args; plain-Python formulas auto-wrapped with `njit()`). Reuses `Graph.compile` (topological order + external-variable discovery) and `utils/preprocessing`'s content-addressed anchor (hash covers each formula's source **and** closed-over values; cached kernels reload cross-process). `CompiledKernel` exposes a bare `.kernel` hot path plus a dict-in/out `.execute` mirroring `CompiledGraph.execute`; generated identifiers are keyword- and newline-safe with minimal deterministic suffixes. v1 omits per-node `cacheable` memoization, incremental `recompute`, `None`-as-value formulas, and node-identity load/combine (use `CompiledGraph`/`Work`). Design + plan under `docs/superpowers/{specs,plans}/2026-06-07-compile-kernel*` (fork-only, not on `main`); tests in `test/core/test_compile_kernel.py`.
